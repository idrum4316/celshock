...
/**
 * GREYFEN — the authored layout.
 *
 * **Forked from Hollowmere and diverging.** Every placement below started as
 * that map's, so the ASCII map and the district commentary still describe the
 * village both share. Change them as this one moves away from it: a district
 * note that has stopped being true is worse than none, because the next author
 * will trust it. The two files are otherwise unrelated — nothing imports across
 * them, and the editor patches each in place (see `src/editor/save.ts`).
 *
