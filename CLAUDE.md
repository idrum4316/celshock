...
- **The version hashes every file's NAME AND CONTENTS.** Hashing the names
  ...
  `index.html` is unhashed and carries the entire HUD stylesheet, so a CSS-only
  change would leave the worker byte-identical, and a byte-identical worker is
  one the browser never updates. The old HUD would be served from cache
  forever.
