...
**The menu stands on a PHOTOGRAPH of the map that is chosen, and it is under the
veil rather than in the card.** `shots/<id>.jpg` is a real screenshot of that
map, taken by `npm run shots` from the vantage `src/ui/mapShots.ts` states
beside it, and choosing another map cross-fades to that one's. It is a root of
its OWN — `#menu-shot`, a child of `#hud` at z-index 9 — and both halves of
that are load-bearing: `showMenu` rewrites the card's markup on every map step,
and a layer removed and re-inserted has no style left to fade FROM; and a child
of `#overlay` paints over its parent's BACKGROUND whatever its z-index, which is
exactly where the veil is. So the picture goes UNDER the shared veil at
`card-menu`'s own density — the two custom properties `.ui-veil` already exposes
for how much of what is behind shows through — rather than growing a scrim of
its own. **A map with no row in that table is not a broken screen**: the menu is
the one it was before there were shots at all.
...
map), the backdrop behind all of it — where the picture comes from, what the
cross-fade waits for and why one veil density has to hold a night village and a
dusk skyline — where the prose in those panels lives, the shared footer and why Back is
