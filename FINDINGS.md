...
## 9. The editor's clearance validator probes a transposed map

**Status:** measured (by reading the indexing against `NavGrid`'s), not yet fixed.

### What was measured

`validateClearance` in `src/editor/validate.ts` derives its world sample as
...
`NavGrid` indexes `cell = cz * dim + cx` throughout — `rasterize`, `link`,
`positionOf` — so `cell % dim` is **cx** and `Math.floor(cell / dim)` is **cz**.
The two are swapped here, so every `obstacles.resolve` / `surfaceAt` probe is
taken at `(z, x)`: the pass walks the right list of surfaces and asks about them
at their mirror image across the map diagonal.

It reads the surface height from the correct `cell`, so the heights are right and
the XZ is wrong — which is why it still returns a plausible-looking count rather
than nothing. Greyfen reports 177 spots today; that number is a measure of a map
that does not exist.

### What is derived and still a hypothesis

That the correct pass would report a *different* number rather than the same one
by luck. Hollowmere and Greyfen are both roughly diagonally busy, so a transposed
probe lands on real geometry most of the time — it just isn't the geometry the
surface belongs to.

### How to settle it

Swap the two lines and re-run the validator on both maps, then eyeball the first
few `at` positions in the editor: a genuine trap is a body-width pinch you can
see. Worth doing before anything trusts the count — this is the pass that would
otherwise have been the natural place to catch a stair whose foot bots cannot
reach, and it could not have caught it.
